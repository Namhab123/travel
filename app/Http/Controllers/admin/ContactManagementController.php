...
class ContactManagementController extends Controller
{
    ...
    public function replyContact(Request $request)
    {
        ...
        try {
            ...
            // Lấy lại số lượng tin nhắn chưa đọc sau khi cập nhật
            $unreadData = $this->contact->countContactsUnread();
            $newUnreadCount = $unreadData['countUnread'];

            return response()->json([
                'success' => true,
                'message' => 'Phản hồi qua email thành công.',
                'newUnreadCount' => $newUnreadCount // Trả về số lượng mới
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
