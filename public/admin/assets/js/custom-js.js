$(document).ready(function () {
    ...
    if ($("#regulatery").length) {
        CKEDITOR.replace("regulatery");
    }
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $("#start_date").val(startDate).removeAttr("readonly").removeAttr("disabled");
                    $("#end_date").val(endDate).removeAttr("readonly").removeAttr("disabled");
                    ...
                    CKEDITOR.instances["regulatery"].on(
                        "instanceReady",
                        function () {
                            CKEDITOR.instances["regulatery"].setData(
                                tour.regulatery
                            );
                        }
                    );
    ...
    function getFormDataImages() {
        ...
        if (formDataImages.length > 5) {
            toastr.error("Chỉ được thêm tối đa 5 ảnh");
            return false; 
        }
        ...
        $("#edit-tour-modal #wizard").smartWizard({
            onLeaveStep: function (obj, context) {
                ...
                if (stepIndex === 1) {
                    ...
                    // Kiểm tra mô tả
                    var regulatery =
                        CKEDITOR.instances["regulatery"].getData();
                    if (!regulatery) {
                        finishStep1 = false;
                        toastr.error("Vui lòng điền mô tả!");
                    }
                    ...
                    // Tính số ngày từ start_date và end_date để kiểm soát timeline
                    const startDateMoment = moment(formDataEdit.start_date, "DD/MM/YYYY");
                    const endDateMoment = moment(formDataEdit.end_date, "DD/MM/YYYY");
                    const diffDays = endDateMoment.diff(startDateMoment, "days") + 1;

                    if (diffDays < 1) {
                        toastr.error("Ngày kết thúc phải sau hoặc bằng ngày bắt đầu!");
                         return false;
                    }
                    maxTimelineDays = diffDays;
                    console.log("Số ngày timeline cho phép:", maxTimelineDays);
    ...
    init: function () {
        this.on("sending", function (file, xhr, formData) {
            ...
            console.log("Sending file:", file.name, "with data:", formData);
        });
        this.on("success", function (file, response) {
            console.log("Upload success:", response);
        });
        this.on("error", function (file, errorMessage, xhr) {
            console.error("Upload error:", errorMessage, xhr.responseText);
            toastr.error("Lỗi tải ảnh: " + (xhr.responseText || errorMessage));
        });
    ...
    function addTimelineEntry() {
        ...
        console.log("maxTimelineDays:", maxTimelineDays);
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    // Cập nhật số lượng tin nhắn chưa đọc trên sidebar
                    $('.badge.bg-green').text(response.newUnreadCount); // Cập nhật số lượng mới
    ...
    e.preventDefault(); // Ngăn gửi form thông thường
